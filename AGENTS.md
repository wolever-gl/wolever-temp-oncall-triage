...
- Move a group to `waiting` only after there is durable evidence that the blocker has been communicated to the owning party. The CLI enforces this from `evidence.jsonl` and `notes.md`; generated `case.md` cannot satisfy the rule. Use communication evidence kinds such as `communication_thread`, `support_thread`, `slack_thread`, `customer_message`, `owner_handoff`, or `ticket` with a durable link when available. If communication has not happened yet, report findings and leave the group `open` unless another state is clearly justified.
