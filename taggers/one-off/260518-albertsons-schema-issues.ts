...
const logEvidence = await audienceLogSchemaEvidence(alert);
...
async function audienceLogSchemaEvidence(alertFact: AlertFact) {
  if (!alertFact.audience_id) return undefined;

  const artifactPath = path.join(process.cwd(), "cases", "artifacts", "260518-albertsons-schema-log-evidence.json");
  let artifact: AudienceLogEvidenceFile;
  try {
    artifact = await Bun.file(artifactPath).json() as AudienceLogEvidenceFile;
  } catch {
    return undefined;
  }

  const audienceEvidence = artifact.audiences?.[alertFact.audience_id];
  if (!audienceEvidence) return undefined;

  return {
    kind: "gcloud_log_evidence",
    matched: true,
    source: artifactPath,
    summary: audienceEvidence.summary ?? `Audience ${alertFact.audience_id} has Quervice schema failure evidence.`,
    data: {
      audience_id: alertFact.audience_id,
      path: audienceEvidence.path,
      exception: audienceEvidence.exception,
      first_seen: audienceEvidence.first_seen,
      last_seen: audienceEvidence.last_seen,
      request_ids: audienceEvidence.request_ids ?? [],
      query_source: artifact.source,
    },
  };
}
